...
* Don't use a toggle to trigger an action such as running a pipeline: use a [button](button).
...
| Confirm first when flipping is destructive or affects others (disabling a shared connection) | Silently cut off every consumer of a pipeline |
