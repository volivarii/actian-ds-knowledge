...
During the `src/`+`dist/` migration, `generate-guideline-stubs.js` writes stubs to BOTH `components/src/guidelines/` AND legacy `components/guidelines/`. After Phase D, only the `src/` path remains.
