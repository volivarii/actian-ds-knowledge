...
machine-read field did not. The action bar is named by its current slug `sticky-footer`; Figma has
renamed it to `action-bar` and that rename can only land alongside the registry, in one branch.
