...
import { incomingFiles } from "../lib/incomingFiles";
...
export function RelationsPanel(props: RelationsPanelProps) {
  ...
  // The rail answers "which files point here", so it counts files. Reference
  // sites are still visible, as a count on the file's own row (#684). Scoping
  // to a section re-derives, so the count always matches the rows below it.
  const { files: incomingByFile, generatedExcluded } = useMemo(
    () => incomingFiles(visibleIncoming),
    [visibleIncoming],
  );
      ...
      {!collapsed && (
        <>
          ...
          <Text size="1" color="gray">
            {/* Qualified only when something was withheld: a bare "(0)" sitting
                above "3 generated files reference this" is the same
                contradiction the empty state had, carried by the number. */}
            Referenced by ({incomingByFile.length}
            {generatedExcluded > 0 ? " editable" : ""})
          </Text>
          {incomingByFile.length === 0 && generatedExcluded === 0 && (
          ...
          {generatedExcluded > 0 && (
            ...
            // With every referrer generated — nine of the sixteen anchors in
            // accessibility/src/components.md are exactly that, because the
            // index never scans _meta.yml — an "also" beside an empty list
            // read as a contradiction, and the empty state read as a lie.
            <Text size="1" color="gray" data-testid="incoming-generated-note">
              {generatedExcluded} generated{" "}
              {generatedExcluded === 1 ? "file" : "files"}{" "}
              {incomingByFile.length > 0
                ? generatedExcluded === 1
                  ? "also references"
                  : "also reference"
                : generatedExcluded === 1
                  ? "references"
                  : "reference"}{" "}
              this, and {generatedExcluded === 1 ? "is" : "are"} regenerated by
              CI rather than edited.
            </Text>
          )}
