...
export function ConflictDialog(props: {
  ...
  octokit: { git: { getBlob(args: unknown): Promise<{ data: { content: string } }> } };
  owner: string;
  repo: string;
  onResolve(resolved: ConflictResolution[]): void;
  onCancel(): void;
}): JSX.Element {
  const { conflicts, mineByPath, octokit, owner, repo, onResolve, onCancel } = props;
  ...
  async function submit() {
    ...
    for (const c of conflicts) {
      ...
      if (choices[c.path] === "merge") {
        const baseRes = await octokit.git.getBlob({ owner, repo, file_sha: c.basedOnSha });
  ...
  return (
    <Dialog.Root open onOpenChange={(o) => { if (!o) onCancel(); }}>
      <Dialog.Content>
        ...
        <Dialog.Description>
          Someone updated these files on <code>main</code>. Choose how to resolve each before submitting.
        </Dialog.Description>
            ...
            <Box key={c.path}>
              ...
              <Flex gap="2" mt="2">
                <Button
                  ...
                  onClick={() => setChoices((s) => ({ ...s, [c.path]: "overwrite" }))}
                >
                ...
                <Button
                  ...
                  onClick={() => setChoices((s) => ({ ...s, [c.path]: "merge" }))}
                >
