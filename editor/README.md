...
Schema-driven editor for the `actian-ds-knowledge` substrate. Authors edit the canonical files (tokens, foundations, components, accessibility, app-context, icon-groups) through forms + rich-text + live preview, and the editor packages each change as a Pull Request against `main`. In the UI the app-context domain surfaces under **Application context** (Products / Entities / Features), separate from the **Design system** sections (where Content nests Writing rules, Patterns, and Product).
