...
test("top-level keys come from the schema, minus what is already written", () => {
  ...
  assert.deepEqual(got.sort(), ["_schema_version", "apps", "properties", "relationships"]);
});
...
test("keys inside an array of objects come from items, through oneOf", () => {
  const got = keyCandidates(ENTITY, ["properties"], ["name"]).map((c) => c.label);
...
test("value candidates come from const", () => {
  const got = valueCandidates(ENTITY, [], "_schema_version").map((c) => c.label);
