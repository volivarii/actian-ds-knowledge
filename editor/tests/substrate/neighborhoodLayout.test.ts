...
const index = buildGraphIndex({
  ...
  edges: [
    ...
    { source: "a11y:contrast", target: "foundation:color", type: "foundations_ref" },
  ],
});

test("focus node is centered and flagged", () => {
  const l = layoutNeighborhood("component:button", index, { width: 600, height: 400 });
...
test("depth 1 includes 1-hop neighbors only; coordinates are integer + deterministic", () => {
  const a = layoutNeighborhood("component:button", index, { depth: 1, width: 600, height: 400 });
  const b = layoutNeighborhood("component:button", index, { depth: 1, width: 600, height: 400 });
  const ids = a.nodes.map((n) => n.id).sort();
  assert.deepEqual(ids, ["a11y:contrast", "category:action", "component:button"]);
  assert.ok(a.nodes.every((n) => Number.isInteger(n.x) && Number.isInteger(n.y)));
  assert.deepEqual(a, b); // pure + deterministic
});

test("edges only connect placed nodes and are deduped", () => {
  const l = layoutNeighborhood("component:button", index, { depth: 1, width: 600, height: 400 });
...
test("edgeTypes filter restricts traversal", () => {
  const l = layoutNeighborhood("component:button", index, { depth: 1, edgeTypes: ["a11y_ref"] });
  assert.deepEqual(l.nodes.map((n) => n.id).sort(), ["a11y:contrast", "component:button"]);
});
