...
function fakeGh(listings: Record<string, Array<{ name: string; type: "file" | "dir" }>>) {
  return {
    repos: {
...
function wrap(node: React.ReactNode) { return <Theme>{node}</Theme>; }

test("Sidebar: renders Foundations + Accessibility entries", async () => {
  render(wrap(<Sidebar octokit={fakeGh(LISTINGS)} pendingPaths={new Set()} activePath={null} onSelect={() => {}} />));
...
test("Sidebar: click dispatches onSelect with full path", async () => {
  const calls: string[] = [];
  render(wrap(<Sidebar octokit={fakeGh(LISTINGS)} pendingPaths={new Set()} activePath={null} onSelect={(p) => calls.push(p)} />));
  await waitFor(() => screen.getByText("foundations.md"));
...
test("Sidebar: shows draft-dot for paths in pendingPaths", async () => {
  ...
  const { container } = render(wrap(<Sidebar octokit={fakeGh(LISTINGS)} pendingPaths={pending} activePath={null} onSelect={() => {}} />));
  await waitFor(() => screen.getByText("foundations.md"));
