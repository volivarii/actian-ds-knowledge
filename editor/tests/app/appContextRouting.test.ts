...
import { isAppContextFile, appContextKindConfig } from "../../src/app/EditorShell";
...
test("isAppContextFile matches only the three per-record kinds", () => {
  ...
  assert.equal(isAppContextFile("app-context/src/entities/data-product.md"), true);
  assert.equal(isAppContextFile("app-context/src/patterns/import-wizard.md"), true);
...
test("appContextKindConfig selects schema/uiSchema/bodyless per kind", () => {
  assert.deepEqual(
    { ...appContextKindConfig("app-context/src/apps/studio.md"), uiSchema: undefined },
    { schemaKey: "app-context-app", bodyless: true, uiSchema: undefined },
  );
