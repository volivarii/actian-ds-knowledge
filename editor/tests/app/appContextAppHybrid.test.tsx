...
const CORE_SCHEMA = JSON.stringify({
  ...
  properties: {
    ...
    sidebar: { type: "array", items: { type: "object", properties: { label: { type: "string" }, id: { type: "string" } } } },
  },
});
...
test("app renders the markdown body editor (not bodyless)", async () => {
  ...
  await waitFor(() => assert.ok(screen.queryByText("App label")), { timeout: 5000 });
  assert.ok(screen.queryByText("Prose body"), "app shows the markdown body editor");
  cleanup();
});
