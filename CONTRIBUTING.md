...
## Phase B note (transient, ~2026-05)

The `src/`+`dist/` split is being landed under parallel-change discipline (`MIGRATIONS.md` Rule 1 in the plugin repo). During this window CI writes BOTH new `dist/` and legacy paths (`foundations/*.json`, `components/registries/*.json`, etc.). Plugin consumers continue to work without changes. Phase D removes the legacy duplicates after a stable migration window. See `docs/superpowers/specs/2026-05-10-knowledge-repo-restructure-design.md` in the plugin repo (gitignored).
