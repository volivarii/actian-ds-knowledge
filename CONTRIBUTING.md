...
|---|---|---|
| A foundation token, rule, or scale | `foundations/src/NN-<slug>.md` per-section files | `foundations-derive.yml` regenerates the hierarchical `foundations/dist/` tree (Pattern H — per-leaf JSONs + `_index.json` per directory + `foundations.bundle.json` roll-up + `.md` verbatim concat) |
