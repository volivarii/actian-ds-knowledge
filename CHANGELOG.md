...
- **The sparse render ratchet: a component must not invent parts the caller did not ask for.**
  ...
  the thirteenth shipped. This gate takes no list at all: it renders every slug in the render
  contract with **no props whatsoever** and counts the elements carrying visible text, then blocks
  when that count rises, per slug and repo-wide. A fallback moves the number whatever shape it is
  written in, because what is read is the markup rather than the source. Verified by mutation: a
  literal fallback reintroduced for `chat-with-ai-steward.Source` (a slot absent from
  `SPECIMEN_PROPS`) reds the ratchet with `chat-with-ai-steward: 8 -> 10` while the omission test
  stays green at 17 passing.
  ...
  `tests/render/helpers/merge-base.js` instead of in two copies. A rise can be legitimate, so there
  is an escape hatch, `ACCEPTED_RISE`, which requires the slug and a reason; the failure message
  names the slug with both counts and states the repo-wide direction in its own terms, and it never
  advises regenerating anything. Current state, for orientation rather than as a target: 44 of 58
  slugs render some text with nothing supplied, 176 text-bearing elements in total, much of it
  legitimate structural chrome.
