...
- **The Claude Design bundle names its cards in their markers and ships a root stylesheet.** ([#725](https://github.com/volivarii/actian-ds-knowledge/pull/725)) Each card's `@dsCard` marker carries its `name` and `subtitle`, and the bundle writes `styles.css` at the project root: the fonts and `render.css`, the same two stylesheets every card already inlines. Checked against the dogfood project with a probe card and a probe stylesheet, read back from the compiled `_ds_manifest.json`: the index keeps a marker's `name`, `subtitle` and `viewport`, and records a root stylesheet in `globalCssPaths` with its custom properties as the system's tokens and its `@font-face` rules as its fonts. Pushed to the dogfood and read back: 85 cards, 77 named from their markers (the 8 declared external cards keep file names); 231 tokens plus the 46 studio and 46 explorer overrides, each carrying a `scope`, with Studio and Explorer recorded as themes; Roboto and Inter as fonts. That read also showed three cards named "Cards" and three named "Tags", because a guideline doc names its family, so a family name shared by more than one rendered component now gives way to the component's humanized slug. It also shows `--zen-font-family-brand` (AllRpungGothic) and `--zen-font-family-mono` (Roboto Mono) with no `@font-face` in the render dist. Before this every card was listed by its file name and the index held no tokens and no fonts, and `register_assets`, which the exporter had been documented as the only naming path, changed nothing on a `"source": "spa"` project. `selfContainedCard()` takes the name and subtitle as an optional sixth argument, so `build-contact-sheet.js`'s five-argument call is unchanged. `bundle-external.json` drops `calendar-date-input` and `rich-text`, removed from the dogfood on 2026-09-24 after `calendar` and `rich-text-froala` were confirmed to read correctly.
...
- **The brand font token is Roboto, the apps' font.** ([#726](https://github.com/volivarii/actian-ds-knowledge/pull/726)) `--zen-font-family-brand` read `AllRpungGothic`: a misspelling of AllRoundGothic, hard-coded in `scripts/tokens/derive-tokens.js` as a figma-only carry-forward that overwrote the Font Family table's own brand row (`HCLTech Roobert`), with a test asserting the misspelling. The face never shipped in the render dist, and Claude Design's index flagged it on 2026-09-24. Decided the same day: brand resolves to Roboto, the apps' font, so `foundations/src/tokens.md` sets it beside text, the derive no longer overrides the table, and `design-guidelines.md` says so (Roobert stays the typeface of marketing and communication materials). `--zen-font-family-mono` stays Roboto Mono for code; it still has no face in the render dist. A test derives the real table and requires text and brand to be Roboto, mono Roboto Mono, and none a carry-forward; restoring the old override fails it.
...
- **Four form components render as real controls instead of empty grey boxes**
  ...
  box. The plugin's own committed baseline
  (`tests/renderers/blank-box-baseline.json`, measured across the authorable vocabulary rather than
  on any one screen) records exactly what that cost: checkbox-group 5, radio-group 5, text-area 3,
  field 2. That is 15 of its 65 blank boxes, and 15 of the 23 that are not the two chart components.

  **Consumers: this improvement has to be banked, it does not land by itself.** The plugin skips
  built slugs when it measures, so once the refresh carries these four its baseline no longer
  describes the output and two gates go red by design, each with a one-command fix:
  `tests/integration/authoring-table-sync.test.js` (regenerate with
  `node scripts/renderers/render-authoring-table.js`) and `tests/renderers/blank-box-budget.test.js`
  (bank with `node scripts/renderers/ds-coverage-report.js --write-baseline`). The docs site needs
  nothing: its component pages are generated by `prebuild`, so the four renders appear on their Code
  tabs on the next build.

  Every structural and colour decision is quoted from the component's own
  `components/dist/anatomy/<slug>.json`, and the notes in the source say which are quotes and which
  are authored. Three things the capture settled that reasoning had got wrong: `field`'s Error
  border is `--zen-color-error-700` (#c12c11), not the `--zen-border-error` (#dc3514) the name
  suggests; Focus is a 2px ring while Active is a 1px border in the same colour, so folding them
  together loses a distinction the design file makes; and the box radius is 6px, not the 4px that
  was guessed. `field` emits `ds-fieldbox` rather than `ds-field`, which `text-input` already owns
  as its outer wrapper, and declares it in `matrix.js` `CSS_OWNERS`.

  Two states the capture records are deliberately not painted. `Hover` sets only a background
  (#fbfbff) that binds to no published token, so it is left unpainted rather than hardcoded, and it
  renders identically to `Default`. `Filled` IS painted, because the capture moves the input text
  from the placeholder colour to the default one and both are published tokens; it is worth naming
  because no gate would have reported it collapsed, `variant-collapse.js` exempting State axes
  wholesale on the grounds that a static fragment cannot show hover.
...
- **The Coverage table is open at rest**
  ...
  second question ("open this one") needs a click before it can be asked has hidden its own verb.
  The figure answers where the system stands, the table answers which row to open, and both are now
  visible. The disclosure stays, so the figure can still be read alone.
  Editor-only: no dist, no schema, no consumer.
...
- **A form save writes the author's own bytes back when nothing changed**
  ...
  cosmetic defect while dropping an edit is data loss. Leaving the batch is decided on the same
  question rather than on bytes alone, because a file REOPENED from the batch is seeded with the
  cart's own bytes: once a reformat had staged it, the assembled file could never equal `main` again,
  so the whitespace-only pull request survived navigating away and back. A corpus guard walks every
  routed `.md` file and asserts it survives its own save byte for byte; removing either shortcut
  turns it red with the original 29 and 1. Editor-only: no dist, no schema, no consumer, and no
