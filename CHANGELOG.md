...
- **Content dist split: per-bucket views beside the global concat.** `derive-content.js` now also
  ...
  splits. Parallel change per MIGRATIONS Rule 1: `global.md` is byte-identical and stays the
  cross-bucket document (root-level ground rules like `global-guidelines.md` appear only there), so
  existing consumers are unaffected. Tests pin bucket partition correctness and per-file dist
  staleness. ([#387])
