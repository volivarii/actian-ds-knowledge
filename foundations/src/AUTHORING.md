...
> **About the `src/` folder:** the repo organizes editable sources under `src/` and CI-generated outputs under `dist/` (visible folder boundary). You only ever edit `foundations/src/foundations.md`. The 8 derived JSONs live in `foundations/dist/` and are regenerated by CI on every PR — never edit them directly.
...
3. It commits the JSON changes back to your branch with the message `chore(foundations): regenerate JSONs from foundations.md + bump patch`.
...
> **Phase B note (transient):** during the `src/`+`dist/` migration window, CI also writes the 8 JSONs to the legacy `foundations/*.json` paths (so existing consumers keep working). Phase D removes the legacy duplicates. You don't need to do anything different — just edit `foundations/src/foundations.md`.
