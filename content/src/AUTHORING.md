...
- **`content/dist/{writing,patterns,product}.md`** — CI-generated by the same `scripts/content/derive-content.js`: per-bucket split views of the same global sections (manifest keys `content.writingMd` / `content.patternsMd` / `content.productMd`), so a consumer that needs only one family reads only that file. Root-level meta files (like `global-guidelines.md`) stay in `global.md` only. Never edit `dist/`.
- **`content/dist/words-to-avoid.json`** — CI-generated by the same `scripts/content/derive-content.js`, parsed from `writing/words-to-avoid.md`. Structured rules (`{ avoid: [tokens], reason, example: { do, dont } }`; advisory rows carry `avoid: []`) for programmatic consumers; `global.md` keeps the prose table for humans. Never edit `dist/` — edit the source table and let CI regenerate.
- **`components/src/<slug>/content.md`** — per-component content guidelines (a different domain — see `components/src/AUTHORING.md`).
- **`foundations/src/<slug>.md`** — primitives, tokens, scales (Design system lead's domain, per-section files; ordering declared in `_order.json`).
- **`tokens/`** — DTCG W3C-format design tokens.
- **`accessibility/src/<slug>.md`** — WCAG 2.2 AA guidance, per-section files (ordering declared in `_order.json`).
- **`app-context/app-context.json`** — per-app patterns, persona, terminology.

## Style basis

All guidelines follow IBM Style conventions and use sentence case throughout. `global-guidelines.md` is the master reference for voice and tone. Words-to-avoid rules are authored in `writing/words-to-avoid.md` and also emitted as structured JSON at `content/dist/words-to-avoid.json` (manifest key `content.wordsToAvoid`) for programmatic consumers.
