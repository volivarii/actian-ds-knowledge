...
| **Content guidelines** (mixed) | Component-scoped content lives in `components/src/<slug>/content.md`; global / cross-cutting topics in `content/src/` (root + `_global/`) + `content-index.md` + `format-spec.md` + `AUTHORING.md` (Content lead) | `content/dist/content.md` (CI-consolidated reference from `scripts/content/derive-content.js`) |
