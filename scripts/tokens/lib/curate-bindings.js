...
function curatePrimitiveBindings(raw) {
  ...
  for (const v of vars) {
    if (v.variableCollectionName !== "Zen colors" || v.variableType !== "COLOR") continue;
    ...
    const path = shade ? `color.primitive.${palette}.${shade}` : `color.primitive.${palette}`;
    map[path] = { variableKey: v.key, scopes: v.scopes || [] };
  }
  return map;
}

module.exports = { curatePrimitiveBindings };
