...
function emitCss(fullTree) {
  ...
  // The old header named tokens/src/derived/tokens.candidate.json, a file that
  // no longer exists, and nothing in it told a reader the file is regenerated.
  // tokens/README.md meanwhile still described tokens.css as a human-frozen
  // snapshot to be edited by hand, which is how an edit gets silently reverted
  // by the next derive.
  const header = [
    "/**",
    " * Actian Design System 2026 — CSS Custom Properties",
    " *",
    " * AUTO-GENERATED - DO NOT EDIT. Regenerated by `npm run derive:tokens`",
    " * (scripts/tokens/derive-tokens.js), which CI runs in foundations-derive.yml.",
    " * Hand edits are overwritten on the next run.",
    " *",
    " * Derived from:",
    " *   foundations/src/color-primitives.md   (the OKLCH shade formula and palette bases)",
    " *   foundations/src/tokens.md             (semantic token mappings)",
    " *   tokens/src/figma-bindings-raw.json    (harvested Figma bindings)",
    " *",
    " * Shade values are COMPUTED from the OKLCH formula, not copied from Figma.",
    " * Figma stores hand-picked hex, so the two can legitimately differ; see the",
    " * `Note on Figma vs Eng` in foundations/src/color-primitives.md.",
    " *",
    " * Prefix: --zen-",
    " */",
  ].join("\n");
