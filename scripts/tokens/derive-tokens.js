...
const { curatePrimitiveBindings, curateSemanticBindings } = require("./lib/curate-bindings.js");
...
function deriveSemanticTree({ primitivesMd, semanticsMd, rawBindings }) {
  // primitivesMd may contain both the primitive tables AND the role/theme tables
  // (caller passes color-primitives.md + tokens.md concatenated at CLI level)
  const primitiveTree = derivePrimitiveTree({ primitivesMd, rawBindings });
  const globalRoles = parseGlobalRoles(primitivesMd);
  const themes = parseThemes(primitivesMd);
  ...
  for (const role of RAMP_ROLES) {
    tree.color[role] = {};
    const actianPalette = (themes.actian && themes.actian[role]) || globalRoles[role];
    if (!actianPalette) continue;
    for (const shade of SHADES) {
      if (!primitiveTree.color.primitive[actianPalette] || !primitiveTree.color.primitive[actianPalette][shade]) continue;
  ...
  for (const s of parseSemantics(semanticsMd)) {
    ...
    let aliasTarget;
    if (m) {
      const actianPalette = (themes.actian && themes.actian[m[1]]) || globalRoles[m[1]] || m[1];
      aliasTarget = `{color.primitive.${actianPalette}.${m[2]}}`;
    } else {
      aliasTarget = `{color.primitive.${s.resolvesTo}}`; // singleton (black/white)
    }
    const value = s.opacity != null ? applyAlpha(R.resolveHex(s.resolvesTo, "actian"), s.opacity) : aliasTarget;
    const tokenPath = `color.${s.group}.${s.name.replace(/\./g, ".")}`;
    const node = semLeaf(value, semBindings, tokenPath, themesHex(R.resolveHex, s.resolvesTo, s.opacity));
...
function main() {
  ...
  const tokensMd = fs.readFileSync(
    path.join(REPO_ROOT, "foundations", "src", "tokens.md"), "utf8");
  ...
  // Semantic tree — pass color-primitives.md + tokens.md concatenated so both
  // parsePrimitives (primitive rows) and parseGlobalRoles/parseThemes (role/theme tables) see their inputs
  const combinedForRoles = primitivesMd + "\n" + tokensMd;
  const semTree = deriveSemanticTree({ primitivesMd: combinedForRoles, semanticsMd: tokensMd, rawBindings });
  fs.writeFileSync(path.join(outDir, "semantics.tokens.json"), JSON.stringify(semTree, null, 2) + "\n");
  const sn = Object.values(semTree.color).reduce((a, v) => a + (v.$type ? 1 : Object.keys(v).length), 0);
  process.stdout.write(`[derive-tokens] wrote ${sn} semantic tokens to tokens/src/derived/semantics.tokens.json\n`);
}
