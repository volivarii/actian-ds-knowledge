...
var writeAtomic = require("../lib/dist-io.js").writeAtomic;
...
function detectSlugCollisions(registriesWithKit) {
  var seen = {};
  registriesWithKit.forEach(function (rk) {
    var comps = (rk.reg && rk.reg.components) || {};
    Object.keys(comps).forEach(function (slug) {
      var c = comps[slug] || {};
  ...
  Object.keys(seen)
    ...
    .forEach(function (slug) {
      ...
      if (
        ...
        slug_collisions.push({
          slug: slug,
          resolved_to: cands[0].kit,
          candidates: cands,
        });
      }
    });
  return { slug_collisions: slug_collisions };
}
...
function derive() {
  ...
  var registriesWithKit = REGISTRY_FILES.filter(function (rel) {
    return fs.existsSync(path.join(ROOT, rel));
  }).map(function (rel) {
    return { kit: path.basename(rel, ".json"), reg: readJSON(rel) };
  });
  var registries = registriesWithKit.map(function (x) {
    return x.reg;
  });
  ...
  // writeAtomic (temp + rename), not a bare writeFileSync: tests re-derive
  // this dist in place while parallel test processes read it, and a truncate
  // window mid-write surfaces as "Unexpected end of JSON input" in CI.
  writeAtomic(outPath, M.stableStringify(out));
  console.log(
    "derive-graph: wrote " +
      out.nodes.length +
      " nodes, " +
