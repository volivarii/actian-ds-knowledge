...
function collectComponentsAndCategories(g, registries) {
  registries.forEach(function (reg) {
    ...
    Object.keys(comps).forEach(function (slug) {
      var c = comps[slug];
      g.addNode({ id: M.nodeId("component", slug), type: "component", title: c.name || slug });
      if (c.category) {
        var catSlug = M.slugify(c.category);
        g.addNode({ id: M.nodeId("category", catSlug), type: "category", title: c.category, provenance: "figma-dskit" });
        g.addEdge({ source: M.nodeId("component", slug), target: M.nodeId("category", catSlug), type: "in_category" });
      }
    });
  });
}
...
function derive() {
  ...
  console.log("derive-graph: wrote " + out.nodes.length + " nodes, " + out.edges.length + " edges → graph/dist/graph.json");
  return out;
}

if (require.main === module) {
  try { derive(); } catch (err) { console.error("derive-graph FAILED:", err.message); process.exit(1); }
}

module.exports = { derive: derive, collectComponentsAndCategories: collectComponentsAndCategories, readJSON: readJSON, ROOT: ROOT };
