...
2. **Every knowledge domain has a `src/` and `dist/` split.** `foundations/`, `components/`, `content/`, `accessibility/` and `app-context/` all do, and none of their `dist/` may be edited. `app-context/dist/` is regenerated by `app-context-derive.yml` from `app-context/src/{apps,entities,patterns,personas}/*.md` + `terminology.yml`.
...
- **`app-context-derive.yml`** — `app-context/src/{apps,entities,patterns,personas}/*.md` + `terminology.yml` → `app-context/dist/app-context.json` + `app-context.bundle.json`
