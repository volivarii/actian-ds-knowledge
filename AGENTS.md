...
- Flat domains (no `src/`+`dist/`): `presentation/`, `app-context/`, `fm-to-ds-map/`. `content/` and `accessibility/` are human-authored but have a `src/` → CI `dist/` derive (never edit `dist/`).
