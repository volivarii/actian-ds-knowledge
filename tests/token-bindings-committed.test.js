...
test("committed token-binding sidecars join the committed anatomy by node-id", () => {
  ...
  for (const f of sidecars) {
    ...
    const an = JSON.parse(fs.readFileSync(path.join(ANADIR, slug + ".json"), "utf8"));
    const own = ownNodeIds(an.root);
    for (const id of Object.keys(sc.byNodeId)) {
      assert.ok(own.has(id), slug + ": sidecar node " + id + " must be a non-instance anatomy node (join/staleness)");
      for (const b of sc.byNodeId[id]) {
        assert.ok(PROPS.has(b.property), slug + " " + id + ": property " + b.property);
...
test("card-for-perimeter committed sidecar carries the card shell facts on the root node", () => {
  const sc = JSON.parse(fs.readFileSync(path.join(DIR, "card-for-perimeter.json"), "utf8"));
  ...
  assert.deepEqual(byProp["background-color"], { property: "background-color", token: "--zen-color-bg-default", grade: "semantic" });
  assert.deepEqual(byProp["padding"], { property: "padding", token: "--zen-spacing-sm", grade: "semantic" });
  assert.deepEqual(byProp["border-radius"], { property: "border-radius", token: "--zen-border-radius-sm", grade: "semantic" });
});
