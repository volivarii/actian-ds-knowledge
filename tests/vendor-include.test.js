...
function emitted() {
  return JSON.parse(
    fs.readFileSync(path.join(ROOT, "vendor-include.json"), "utf8"),
  );
}
...
test("include contains the knowledge domains", function () {
  ...
  ["accessibility", "components", "content", "foundations"].forEach(
    function (d) {
...
test("include EXCLUDES tooling + contributor docs (by construction)", function () {
  ...
  [
    ...
    assert.ok(
      !inc.includes(x),
      "tooling/contributor entry leaked into include: " + x,
    );
  });
});
