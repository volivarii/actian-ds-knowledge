...
test("RENDER_SLUGS lists the 35 gallery slugs including button", function () {
...
test("button matrix comes from the curated override (Intent x Emphasis)", function () {
  ...
  assert.ok(labels.some(function (l) { return /critical/i.test(l); }), "has a Critical cell");
  assert.ok(cells.every(function (c) { return c.props && "Label" in c.props; }), "cells carry a Label prop");
});

test("findComponent is ds-first-wins for a dual-kit slug", function () {
  var e = M.findComponent("calendar");
  assert.ok(e && e.variants, "calendar resolves to a registry entry with variants");
});
