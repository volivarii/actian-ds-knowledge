...
// tag-read-only and replaced its Color axis with a single Type axis. This test
...
// registry says which axis tag-read-only publishes and which values it carries,
...
test("readAppearance: tag-read-only's variants are keyed by the axis the registry publishes, and every colour group names published values", function () {
  ...
  assert.equal(
    axes.length,
    1,
    "tag-read-only is expected to publish exactly one axis, got " +
      JSON.stringify(axes),
  );
  var axis = axes[0];
