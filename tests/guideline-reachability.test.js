...
const UNREACHABLE = {
  ...
  "chat-with-ai-steward":
    "LEFT the registry when the Figma component was unpublished on 2026-08-26. It is an OLD version being rebuilt (Kristina/Chetan), archived in the DS Kit file rather than deleted, and expected back once the new one ships. The authored guideline is kept guidance-only, like search-filters; the render leaf and its ds-steward CSS were retired in the same pass because they reproduce the version being replaced and no capture can verify them any more. Delete this exception when the component is republished: the next sync restores the registry entry on its own.",
  "upload-file":
