...
test("slugify normalizes labels (incl. punctuation) to the defaults slugs", function () {
  assert.equal(M.slugify("Form (input & selection)"), "form-input-selection");
  assert.equal(M.slugify("Data Display"), "data-display");
  assert.equal(M.slugify("Breakpoint, grid & structure"), "breakpoint-grid-structure");
});

test("nodeId namespaces by type so cross-domain slugs don't collide", function () {
  assert.equal(M.nodeId("a11y_criterion", "color-contrast"), "a11y:color-contrast");
  assert.equal(M.nodeId("foundation_section", "color-contrast"), "foundation:color-contrast");
  assert.notEqual(M.nodeId("a11y_criterion", "color-contrast"), M.nodeId("foundation_section", "color-contrast"));
});
...
test("GraphBuilder dedups nodes by id and sorts deterministically", function () {
  ...
  g.addEdge({ source: "component:a", target: "category:x", type: "in_category" });
  var out = g.build();
  assert.deepEqual(out.nodes.map(function (n) { return n.id; }), ["component:a", "component:b"]);
  assert.deepEqual(out.edges.map(function (e) { return e.type; }), ["in_category", "related"]);
...
test("schemas/graph.json validates a well-formed graph and rejects a malformed one", function () {
  var schema = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "schemas", "graph.json"), "utf8"));
  var validate = new Ajv({ allErrors: true }).compile(schema);
  var good = { _schema_version: 1, _meta: { auto_generated: true, generator: "x", do_not_edit: "y" },
  ...
  var bad = { _schema_version: 1, _meta: { auto_generated: true, generator: "x", do_not_edit: "y" },
    nodes: [{ id: "a11y:x", type: "BOGUS", title: "X" }], edges: [] };
