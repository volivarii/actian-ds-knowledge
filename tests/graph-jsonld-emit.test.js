...
before(function () { derive(); }); // regenerate both dist files from current substrate
...
test("graph.jsonld is valid JSON-LD with @context, _meta stamp, and @graph", function () {
  var ld = JSON.parse(fs.readFileSync(path.join(ROOT, "graph/dist/graph.jsonld"), "utf8"));
...
test("graph.jsonld is lossless vs graph.json (one @graph entry per node + edge)", function () {
  ...
  var ids = new Set(ld["@graph"].filter(function (o) { return o["@id"]; }).map(function (o) { return o["@id"]; }));
  g.nodes.forEach(function (n) { assert.ok(ids.has(n.id), "missing node " + n.id); });
});
