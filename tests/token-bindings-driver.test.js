...
export default function TagX({ className, status = "Fail" }: TagXProps) {
  ...
  return (
    <div className={className || \`\${String.raw\`gap-[var(--spacing\\/spacing-2xs,4px)] \`}\${isSuccess ? "bg-[var(--color-bg-warning,#fff9e5)]" : "bg-[var(--color-bg-default,white)]"}\`} id={isSuccess ? "node-1_2" : "node-1_1"}>
      <p className="text-[color:var(--color-text-tertiary,#50505d)]" data-node-id="1:3">x</p>
    </div>
  );
}`;
...
test("driver canonicalizes axis names from the anatomy root and joins root bindings", () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      slug: "tag-x",
      root: { id: "1:1", kind: "container", name: "Status=Fail", children: [{ id: "1:3", kind: "text" }] },
    }),
  );
  const res = harvest.run({
    captureDir: t.cap, tokensPath: TOKENS, anatomyDir: t.anat, outDir: t.out,
    slugs: ["tag-x"], harvestedAt: "2026-07-02T00:00:00Z",
  });
  const doc = res.written["tag-x"];
  assert.deepEqual(doc.variantDefaults, { Status: "Fail" }); // canonical casing, not "status"
  const bgs = doc.byNodeId["1:1"].filter((b) => b.property === "background-color");
  ...
  assert.deepEqual(doc.byNodeId["1:3"], [
    { property: "color", token: "--zen-color-text-tertiary", grade: "semantic" },
  ]);
});
...
test("driver skips with a reason when the anatomy root is not among the set root ids", () => {
  ...
  fs.writeFileSync(
    path.join(t.anat, "tag-x.json"),
    JSON.stringify({ slug: "tag-x", root: { id: "9:9", kind: "container", name: "Status=Fail", children: [] } }),
  );
  const res = harvest.run({
    captureDir: t.cap, tokensPath: TOKENS, anatomyDir: t.anat, outDir: t.out,
    slugs: ["tag-x"], harvestedAt: "2026-07-02T00:00:00Z",
...
test("coverage merges pre-existing on-disk sidecars (widening keeps old rows)", () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ _schema_version: 1, slug: "old-slug", byNodeId: { z: [{ property: "color", token: "--zen-x", grade: "primitive" }] } }),
  );
  fs.writeFileSync(path.join(t.cap, "tag-x.design-context.txt"), SET_TEXT);
  fs.writeFileSync(
    path.join(t.anat, "tag-x.json"),
    JSON.stringify({ slug: "tag-x", root: { id: "1:1", kind: "container", name: "Status=Fail", children: [] } }),
  );
  harvest.run({
    captureDir: t.cap, tokensPath: TOKENS, anatomyDir: t.anat, outDir: t.out,
    slugs: ["tag-x"], harvestedAt: "2026-07-02T00:00:00Z",
...
test("parseArgs throws on a dangling flag value", () => {
  ...
  assert.deepEqual(harvest.parseArgs(["node", "x", "--slugs", "a,b"]).slugs, ["a", "b"]);
});
...
test("canonicalAxes parses single and multi-axis root names", () => {
  ...
  assert.deepEqual(harvest.canonicalAxes("Type=Item, State=Default"), { type: "Type", state: "State" });
