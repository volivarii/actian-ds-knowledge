...
const { parsePrimitives } = require("../scripts/tokens/lib/parse-primitives.js");
...
test("parses shaded rows, stripping bold + status emoji", () => {
  const out = parsePrimitives(MD);
  assert.ok(out.find((t) => t.palette === "royal-blue" && t.shade === "25" && t.hex === "#F3F5F9"));
  assert.ok(out.find((t) => t.palette === "royal-blue" && t.shade === "500" && t.hex === "#0F5FDC"));
});

test("excludes warm-grey (reconciliation allowlist)", () => {
  const out = parsePrimitives(MD);
  assert.equal(out.find((t) => t.palette === "warm-grey"), undefined);
});

test("throws on malformed hex", () => {
  const bad = "| 50 | `--zen-color-green-50` | `#ZZZ` | `x` |";
  assert.throws(() => parsePrimitives("#### Green\n| Shade | Token | Hex | OKLCH |\n" + bad));
});
